...
type subtitleStreamFinder interface {
	find(pathToVideo string) (MediaInfo, error)
}

type subtitleStreamExtractor interface {
	extract(pathToVideo, streamIndex string) (*os.File, error)
}
...
func (s *jsonStore) SubsListHandlerFunc() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		info, err := s.subStreamFinder.find(item.Path)
		if err != nil {
...
func (s *jsonStore) SubsHandlerFunc() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		subs, err := s.subStreamExtractor.extract(cacheFile.Path, sid)
		defer func() {
			closeErr := subs.Close()
			if closeErr != nil {
