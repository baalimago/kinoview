...
if ! mv "$tmp_file" $INSTALL_DIR/kinoview; then
  echo "Failed to move the binary to $INSTALL_DIR/kinoview, see error above. Try making sure you have write permission there, or run 'mv $tmp_file <desired-position>'."
  exit 1
fi

echo "kinoview installed successfully in $INSTALL_DIR, try it out with 'kinoview h'"
